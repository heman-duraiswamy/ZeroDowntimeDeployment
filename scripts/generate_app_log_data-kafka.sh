...
do
    ...
    ## generate random number between 1 and 25 for normal state
    ## generate random number between 1 and 15 for bad state
    ...
    echo `date`"|"$SERVER_NAME"|"$APP_NAME"|"$APP_VERSION"|"$msg_type"|log msg "$x2";;" >> ../generated_logs/applog-$SERVER_NAME.log
    /usr/hdp/current/kafka-broker/bin/kafka-console-producer.sh --broker-list sandbox.hortonworks.com:6667 --topic logdemo < ../generated_logs/applog-$SERVER_NAME.log
    rm -fr ../generated_logs/applog-$SERVER_NAME.log
